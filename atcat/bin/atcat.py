...
# import subprocess
# atcat = subprocess.Popen(["/bin/bash", "-i", "-c", "nuke -x scriptpath"])
# atcat.communicate()
...
# warning!  don't let an attacker rm -rf you with this. :)
...
def cmd(cmd, stdin=None, stdout=subprocess.PIPE, stderr=None, shell=False):
    ...
    if inputdata:
        if type(inputdata) is str:
            ...
            raise Exception("atcat.cmd(): stdin must be str or file-like")
